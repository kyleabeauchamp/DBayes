...
qH = pymc.Uniform("qH", 0.40, 0.5, value=0.417, observed=True)
sigma = pymc.Uniform("sigma", 0.31, 0.325, value=0.314)  # 0.31507524065751241
epsilon = pymc.Uniform("epsilon", 0.5, 1.0, value=0.635968, observed=True)
sigmaH = pymc.Uniform("sigmaH", 0.06, 0.12, value=0.1)
...
def calc_density(qH, sigma, epsilon, sigmaH, epsilonH, r0, theta, temperature):
    x = water_lib.build(system, positions, mmtop, temperature * u.kelvin, pressure, qH, sigma, epsilon, sigmaH, epsilonH, r0, theta, stderr_tolerance=density_error)
