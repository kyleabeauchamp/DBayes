...
data.pivot_table(index=keys, columns=["temperature"], values=["density"])
...
predict = lambda q0, sigma0, sigma1, epsilon0, epsilon1, temperature: np.array(models[temperature].predict([q0, sigma0, sigma1, epsilon0, epsilon1], eval_MSE=True))[:, 0]
#predict = lambda q0, sigma0, temperature: np.array(models[temperature](q0, sigma0))
predict(sigma0=0.2, sigma1=0.2, epsilon0=1.0, epsilon1=1.0, q0=0.5, temperature=320)
